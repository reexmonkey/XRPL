...
namespace XRPL.Core.Domain.Responses
{
    ...
    {
        /// <summary>
        /// The amount the account accepting the offer receives, as a <see cref="string"/> representing an amount in XRP, or a token specification object (<see cref="FungibleToken"/>).
        /// </summary>
        [DataMember(Name = "taker_gets")]
        public TGets? TakerGets { get; set; }

        /// <summary>
        /// The amount the account accepting the offer provides, as a <see cref="string"/> representing an amount in XRP, or a token specification object (<see cref="FungibleToken"/>).
        /// </summary>
    ...
    /// <para/> The account accepting the offer provides a <see cref="FungibleToken"/> object representing the amount in a fungible token specification.
    /// </summary>
    public sealed class XrpForTokenAccountOffer : AccountOffer<string, FungibleToken>
    {
    }
    ...
    /// Specifies an an offer made by an account that is outstanding as of the requested ledger version.
    /// <para/>If the number of offers is large, only returns up to limit at a time.
    /// <para/> The account accepting the offer receives a <see cref="FungibleToken"/> object representing the amount in a fungible token specification.
    /// <para/> The account accepting the offer provides a <see cref="string"/> value representing the amount in XRP.
    /// </summary>
    public sealed class TokenForXrpAccountOffer : AccountOffer<FungibleToken, string>
    {
    }
    ...
    /// Specifies an an offer made by an account that is outstanding as of the requested ledger version.
    /// <para/>If the number of offers is large, only returns up to limit at a time.
    /// <para/> The account accepting the offer receives a <see cref="FungibleToken"/> object representing the amount in a token specification.
    /// <para/> The account accepting the offer provides a <see cref="FungibleToken"/> representing the amount in a fungible token specification.
    /// </summary>
    public sealed class TokenForTokenAccountOffer : AccountOffer<FungibleToken, FungibleToken>
    {
    }
}